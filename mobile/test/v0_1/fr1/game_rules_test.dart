...
void main() {
  ...
  test('built-in copy matches the server rules in appsettings.json', () {
    ...
    final autoEnd = server['AutoEnd'] as Map<String, dynamic>;
    ...
    expect(defaultGameRules.minLoopAreaSquareMeters, loop['MinAreaSquareMeters']);
    expect(defaultGameRules.gpsAccuracyThresholdMeters,
        (server['Gps'] as Map)['AccuracyThresholdMeters']);
    expect(defaultGameRules.autoEndIdleMinutes, autoEnd['IdleMinutes']);
    expect(defaultGameRules.autoEndVehicleSpeedKmh, autoEnd['VehicleSpeedKmh']);
    expect(defaultGameRules.autoEndVehicleMinutes, autoEnd['VehicleMinutes']);
    expect(defaultGameRules.safetyAlarmDelaySeconds,
        (server['SafetyAlarm'] as Map)['DelaySeconds']);
  });

  test('the app never knows anti-cheat or gap numbers', () {
    final keys = defaultGameRules.toJson().keys.join(',').toLowerCase();

    expect(keys, isNot(contains('maxspeed')));
    expect(keys, isNot(contains('gap')));
